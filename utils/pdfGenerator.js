...
async function generateLessonReportPDF(reportData, classData, students = null) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Orange brand palette for comment sheet PDFs
            const ORANGE       = '#FF8C42';
            ...
            const addField = (label, content) => {
                ...
                // Label row (orange text, bold)
                if (hasJapanese) {
                    const parenIdx = label.search(/\([\u3000-\u9FFF\uF900-\uFAFF]/);
                    if (parenIdx > 0) {
                        doc.fontSize(10).font('Helvetica-Bold').fillColor(ORANGE_DARK)
                           .text(label.slice(0, parenIdx), MARGIN, fieldY, { continued: true });
                        doc.font('NotoJP-Bold').text(label.slice(parenIdx));
                    } else {
                        doc.fontSize(10).font('NotoJP-Bold').fillColor(ORANGE_DARK)
            ...
            // ══════════════════════════════════════════════════════════════════
            // PAGE HEADER — orange bar with school name & report type
            // ══════════════════════════════════════════════════════════════════
            const HDR_H = 80;
            ...
            doc.fontSize(12).fillColor(ORANGE_LIGHT)
            ...
            infoItems.forEach((item, i) => {
                ...
                doc.fontSize(8).font('Helvetica-Bold').fillColor(ORANGE_DARK)
